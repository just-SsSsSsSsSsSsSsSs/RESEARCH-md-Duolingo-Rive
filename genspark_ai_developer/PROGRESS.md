...
- resume-check (reset #22, 2026-09-26): remote branch @ 36094cf = K1 (owl.motion.json) + K2 (engine/physics.js) + K3 (engine/motion.js, engine/flight.js) present and parse-checked. K4 `sandbox/engine/states.js` was being written at the reset and is NOT on the branch -> rewritten now. K5-K7 pending per the frozen plan. Gists unchanged (5b0800c3 / 555a2147). Owl only; `app/` untouched; rig.js v1 untouched.
- K4 done (2e07ab6, 6a3989c): states.js validated - owl spec ok, fake spec by data ok, bad spec catches 4 problems. K5 done (56291b3): ?engine=v2|v1, spec load + validateSpec, states.fire wiring, trace overlay, slow-mo via shared engine clock, sw g5-4. K6 in progress: measure.py --engine + jank + idle-loop guard (v2: p95 16.7 ms, jank 0-0.84% at 1/3/5, idle loops 0, heap delta <0.8 MB; run-to-run jank at 5 owls varies 0-3.7% on shared CPU). Fixes found by proofs: residual bank on perch (level-off), stale drive rAF handle, physics write coalescing (-45% writes). Next: G2-G8 frames/videos, G10 test file, K7 report + PR.
- resume-check (reset #23, 2026-09-26): branch @ 9813612 = K1-K5 + measure.py (jank/idle-loop) + proofs.py v0 present. Lost in reset: proofs.py fixes (full-page clips because the stage is below the fold; fire-and-forget triggers because pg.evaluate awaited the flight promise) and all proof outputs -> redo now. Then G10 test file, K7 report, PR. Gists unchanged (5b0800c3 / 555a2147). Owl only; app/ untouched.
- K6 done: proofs.py (7b699dd) -> samples/proofs/{g2,g3,g5 strips, g4 trace, g7 edge zoom, g8 v1/v2, proofs.json, v2_owl_showcase.webm 26 s}; independent video review confirms crouch->arc->squash->settle->celebrate->think->talk->slow-mo, no glitches. G10 test node PASS 10/10 (sandbox/tests/g10_fake_character.mjs). measure v2: p95 16.7 ms, jank <1%, idle loops 0, heap +0.67..0.78 MB. Next: K7 report in RESEARCH.md + matrix + risk log, guards, PR.
- K7 done: RESEARCH.md "Gate 5 cycle 4" 10-point report + 12-principles matrix + before/after table + risk log. Guards: family links PASS, Zero-Emoji on added lines PASS (legacy headers in PROGRESS/RESEARCH predate this cycle), protected files SAME, app diff 0. **PR #10 open: https://github.com/just-SsSsSsSsSsSsSsSs/RESEARCH-md-Duolingo-Rive/pull/10** (25 commits, review only - no merge without owner). Preview (sandbox, temporary): https://8080-i1en3d190wb9xjmew13bt-2e1b9533.sandbox.novita.ai/sandbox/index.html?engine=v2 (compare `?engine=v1`; `&trace=1` shows arcs). Frozen plan K1-K7 complete. Next: owner review of PR #10 + proofs; then Gate 6 decision (adapter into app/). Owl only; no other characters.
- resume (2026-09-26, same session): new governing gist a402871d rev 71391abf (K8 SFX/VFX directive + 3 corrections + Foley). Evidence check of its claim "proofs.json state=idle while flying=true -> states.js unproven": TRUE as an observation, root cause = proofs.py called rig.flyBy() directly for slow-motion capture (bypassing CharacterStates), not a broken machine (video/smoke used states.fire). Action order per gist: (1) fix state telemetry in proofs via states.fire + record state per frame -> (2) research SFX (procedural Web Audio, Foley, autoplay policy) -> frozen plan K8 -> execute. No SFX before (1).
- Research K8 (2026-09-26, sources L1): Chrome autoplay blog (Web Audio under policy since Chrome 71; AudioContext starts suspended until user gesture; `--autoplay-policy=no-user-gesture-required` bypass exists -> headless proofs of audio are invalid); MDN `navigator.userActivation` (Baseline 2023-11) gives hasBeenActive/isActive -> testable in code; iOS: mute switch silences Web Audio but not <audio> (feross/unmute-ios-audio technique: play a short silent <audio> on first gesture); ZzFX (MIT, <1 KB, 20 params) = prior art for procedural SFX in web games -> we do NOT claim procedural synthesis as new; claim = Sound DNA in the same Character Spec, validated by validateSpec, zero assets. Perception: Fujisaki & Nishida 2005 (Exp Brain Res) and PMC4451240 (~79-111 ms detection thresholds) -> our cap 40 ms, target 1 frame is stricter than perception.
- FROZEN PLAN K8 (SFX + VFX, owl only, branch sandbox/a1-gate5-art-sw, PR #10 stays open and receives the commits):
  - K8.0 telemetry fix: proofs.py drives flights through `states.fire('move:to')` (not rig.flyBy) and records `states.state` per frame; add `states.history` to proofs.json -> closes the gist's "idle while flying" finding.
  - K8.1 spec: `owl.motion.json` gains `sound` (Sound DNA: baseHz, flapHz range, noise colour, filter Q, envelope shape, wobble) and per-state `sfx` + `vfx` fields, plus new state `rest` (event `idle:long` -> rest, Zzz) and `settle` cue; `validateSpec` checks sfx names exist in the synth catalogue and vfx names in the VFX catalogue.
  - K8.2 `engine/sound.js`: procedural Web Audio synth (osc + noise + biquad + ADSR), no assets; `SoundBus` with master gain, mute toggle, duck (-8 dB while `say()`), polyphony <= 3 and 120 ms same-cue guard, priority; scheduling on `ctx.currentTime` with lookahead; slow-mo aware via engine clock rate (playbackRate on buffer sources / stretched envelopes, or mute if rate < 0.5); unlock only inside a synchronous gesture handler (`pointerdown`) + silent <audio> kick for iOS; fail-safe: any error -> visuals continue, hud note.
  - K8.3 `engine/vfx.js`: lightweight VFX on one shared SVG layer per stage: landing dust puff (3-5 circles, transform/opacity only), speed lines during takeoff/cruise, sparkle stars on celebrate, question mark on think, Zzz on rest, eye glint on blink; all reduced-motion aware, DOM nodes pooled and removed after use.
  - K8.4 wiring: `CharacterStates` emits `cue` events (enter/phase) -> sound + vfx listen; flight phases from flight.js (`anticipate`, `takeoff`, `flapBeat`, `land`, `settle`) fired as cues with engine-clock timestamps.
  - K8.5 measurement: `sandbox/sfx_measure.py` -> `sfx_sync.json` (per cue: t_audio_scheduled - t_visual_cue, target <= 16.7 ms, cap 40 ms; measured on one clock via `ctx.getOutputTimestamp()`/`performance.now()`), `sfx_mix.json` (OfflineAudioContext render of every cue and the busiest overlap: peak dBFS <= -3, zero clipping, duck >= 8 dB), `sfx_polyphony.json` (5 owls: max concurrent <= 3, no same-cue < 120 ms), RAM/jank/idle-loop rerun of measure.py; autoplay proof = node/Playwright test asserting the context is created inside the gesture handler (state after first pointerdown = running) with the policy NOT bypassed (default headless flags keep the gate; document that a real-device check is owner-side).
  - K8.6 docs: RESEARCH.md K8 report (sources above, market table: audio files vs procedural vs Howler/ZzFX), principles matrix rows 6/7/8/12 updated, risk log (iOS mute switch, autoplay, slow-mo), PROGRESS, PR #10 description update. No new PR.
- resume-check (reset #24, 2026-09-26): origin/main @ 05fc33f = PR #10 MERGED by owner (K1-K7 cinematic engine v2 now on main). Branch @ 914d392 = main + K8.0 (telemetry fix) + K8.1 (Sound DNA spec) + K8.2 (engine/sound.js), all present. K8.3 `engine/vfx.js` was being written at the reset -> NOT on branch -> redo now. K8.4-K8.6 pending per the frozen plan. PR #10 is closed by merge, so K8 will need its own review PR at the end (plan updated: "PR #10 stays open" no longer holds). Gists unchanged (5b0800c3 / 555a2147 / 71391abf).
- resume-check (reset #25, 2026-09-26): branch @ a6957c2 = K8.0-K8.4b present (sound.js, vfx.js, foley.js, cue bus). Lost: K8.4c demo wiring (index.html Foley/mute/gesture unlock, sw g5-5) and two fixes found by the smoke test (sound.js zero lookahead: measured offset was 37-44 ms with the 5 ms lookahead; foley.js resolves 'after'/'settle' cues to the state that just ended). Smoke result before reset: autoplay gate honoured (status locked before gesture, running after a real click, 0 played without gesture), 22 cues played, 0 dropped, no console errors. Redo now, then K8.5 measurements.
- resume-check (reset #26, 2026-09-26): branch @ cc5cb5b = K8.0-K8.5 script present (sound/vfx/foley/cue bus, demo wiring, sfx_measure.py v1). Lost: sync-telemetry split (sound.js log: callDelayMs / scheduleErrMs / deviceLatencyMs / offsetMs; sfx_measure.py controlled vs device) and all samples/sfx/*.json. First full run before reset: autoplay PASS (locked -> running after one click, 0 cues without gesture), slow-mo PASS (0 played / 2 muted at x0.25), mix PASS (loudest -14.4 dBFS, overlap -12.9, 0 clipped), polyphony PASS (max 3, min gap 130.6 ms, 59 dropped by guards), sync raw offset 30-44 ms of which device render-ahead = 40 ms (base 10 + output 32) and controlled = 0.2-3.3 ms. Redo split + rerun + commit samples, then K8.6 report + PR.
- resume-check (reset #27, 2026-09-26): branch @ dd436ae = K8.0-K8.5 complete incl. samples/sfx/*.json (summary: all 7 gates PASS) and perf rerun with Foley on. Lost: K8.6 report in RESEARCH.md (was being appended at the reset) -> rewritten now from the committed JSON numbers. Then guards + review PR for K8 (PR #10 already merged, so a new PR is required).
- resume-check (reset #28, 2026-09-26): branch @ 00f32ab = K8.0-K8.6 all present on the branch (engine/sound.js, vfx.js, foley.js, samples/sfx/*.json all PASS, RESEARCH.md "Gate 5 cycle 5" report, merge with origin/main 05fc33f). Nothing lost this time; app diff 0, protected files SAME. Remaining: live preview + closing line.
- K8 done (K8.0-K8.6): all 7 SFX gates PASS (sandbox/samples/sfx: autoplay real-gesture, sync controlled <= 16.7 ms and cap 40 ms, slow-mo mute, mix -3 dBFS/0 clipped, polyphony 3 with 5 owls, total audible under 65 ms), perf PASS with Foley on (1/3/5 owls jank 0/0/0.5 pct, idle loops 0), G10 PASS 10/10, report "Gate 5 cycle 5" in workspace--RESEARCH/RESEARCH.md, guards PASS (family links, app untouched, protected SAME, zero emoji). PR #11 open for review only: https://github.com/just-SsSsSsSsSsSsSsSs/RESEARCH-md-Duolingo-Rive/pull/11 . Preview (sandbox, expires with the sandbox): https://8080-infv5ua5eyfsjqmb406t1-3c7ff1b5.sandbox.novita.ai/sandbox/index.html?engine=v2 (click once to unlock audio; mute button in the engine bar; ?sfx=0 disables Foley; ?slowmo=1 slow motion; ?trace=1 path trace). Next: owner listens on a real phone (Chrome Android + iPhone incl. mute switch, unverified in headless), then approves/merges PR #11. Gate 6 (adapter into app/) stays closed until owner approval. Owl only, branch sandbox/a1-gate5-art-sw only.
- gist a402871d rev 49d30cd9 (2026-09-26T18:26Z) verified before acting: (a) "PR #11 merged" = TRUE (GitHub API: merged 18:10:12Z by just-SsSsSsSsSsSsSsSs, the owner account = the only account behind GITHUB_TOKEN; repo allow_auto_merge=false; PR auto_merge=null; no merge call in this session's tool log). Honest answer: the agent did not merge and the platform has no auto-merge; the merge was made with the owner's account. Language rule accepted: no more "PR open, merge is the owner's". Safety triple after merge: app/ diff vs main = 0 lines; Gate 6 closed (no adapter in app/); feature flags live (?engine=v1|v2, ?sfx=0, ?slowmo, ?trace). (b) six "coldness" causes checked in owl.motion.json: bankLimit 24 TRUE; holdMs once TRUE; armL/armR mirror + legL==legR TRUE; gains all self-velocity TRUE; eyes delayMs 60 after head TRUE; ponder/jumpJoy shape in states.js/motion.js TRUE. (c) Gate 5.5 open items checked: proofs.json has no sagitta, no anticipation_ms; polyphony has no dropped_by_cue; autoplay shows userActive:true before gesture; measure has no p99/max/CPU throttle -> all 6 + 3 notes are real. (d) Duolingo blog quotes: 8/8 found verbatim at blog.duolingo.com/world-character-visemes (fetched today). (e) Rive sizes measured from unpkg 2.43.1: canvas-lite 457 KB gz, webgl2 1012 KB gz vs our engine+spec 22 KB gz. (f) New fact from repo: owl_p2.svg = 12 raster webp parts, 0 paths -> path morph needs art decision; recorded in ADR-002.
- FROZEN PLAN K9 (acting layer, owl only, branch only, Gate 6 closed; commit+push after every chunk):
  K9.0 ADR-002 (A poses+flex / B canvas rig / C rive) with measured sizes -> sandbox/docs/ADR-002-*.md. DONE this line.
  K9.1 Gate 5.5 closure pass (before new motion): proofs.py adds sagitta_px/sagitta_pct/min_radius per arc, anticipation_ms measured from squash timeline, ghost-impulse named with cap (armL sign flip) or fixed; measure.py adds p99/max frame + CPU x4 throttle run + env block; sfx_measure.py adds dropped_by_cue + priority-1 zero-drop check, autoplay rerun on pristine context with userActivation logged per step, sync adds tVisual_effective (= tVisual + next rAF present estimate) and a documented outputLatency compensation decision. Outputs: proofs.json, measure_*_v2.json, samples/sfx/*.json.
  K9.2 Pose sheet gate G11: spec "poses" (6 signature poses: alert, charged, launch, puzzled, triumph, oops) as joint transforms + eye/lid/mouth; tools/pose_sheet.py renders 6 stills at DPR2 + a side-by-side sheet -> samples/proofs/g11_pose_sheet.png; honest per-pose verdict in PROGRESS.md. No motion before this gate is looked at by the owner.
  K9.3 Performance layer in data: spec "beats" per state (t, pose, hold, snap, ease), "asymmetry" (leftRightDelayMs 8-45, gainJitter), hierarchy.eyes.leadMs (eyes lead, head follows, body last) replacing delayMs, "intent" (look-at-target before takeoff, pre-wince before wrong, wink before chime); engine/acting.js applies beats via existing WAAPI layers; validateSpec rejects unknown pose/beat; flag ?acting=0. A/B proof for eye-lead alone (before/after strip).
  K9.4 Owner vision performances (escalation small/medium/large, humour on the owl, reduced-motion): triumph (roll unlocked: flight.rollDeg unlimited + loop path type + tangent-following + level-off before landing + wink + hearts/stars), showoff flight (loop-de-loop + dramatic landing + dust + speed lines), puzzled (spiral gaze, dramatic head tilt, thought bubble VFX), oops (comic recoil + jiggle, owl is the surprised one), idle moving-hold with named distributed impulses.
  K9.5 Flex (limited): 3-segment wing via sliced raster or skew/scale flex, measured cost; active owl only if p95 rises; edge width proof at DPR1/DPR2.
  K9.6 G12 watch test kit (10 s random-order clips v2 vs reference still) + G13 declared budgets table (snap_ms, hold_ms, settle_ms, roll_deg, asymmetry_ms) + sound identity doc + report "Gate 5 cycle 6" in RESEARCH.md with provenance tags + safety triple. No PR language about ownership; report merge state as fact.
- resume-check (reset #29, 2026-09-26): branch @ 0a3391b = K9.0 done (ADR-002 with measured Rive sizes, gist 49d30cd9 verified, frozen plan K9.0-K9.6). Lost in the reset: K9.1a edits (sound.js droppedByCue/playedByCue + measured tVisualEffective via rAF; sfx_measure.py pristine context + userActivation per step + offset_effective + dropped_by_cue + top-priority zero-drop gate) - they were written but the commit was interrupted. Redo K9.1a now and commit before running.
- resume-check (reset #30, 2026-09-26): branch @ 1b95b9d = K9.0 (ADR-002) + K9.1a (sfx: per-cue drops, measured present-time sync, pristine autoplay, headed xvfb mode, samples rerun: all gates PASS, top-priority never dropped by polyphony, effective-sync median PASS headed) + K9.1b (proofs: sagitta 17-24 pct of chord, min radius 70-74 px, anticipation measured 206.1 ms vs drawn 206 ms -> declared 150-250 confirmed, secondary overshoot named ratio 0.089 <= cap 0.15). Lost: K9.1c measure.py edit (p99/max, CPU x4 throttle, env block) - uncommitted at the reset. Redo K9.1c now, commit before running.
- K9.1 done (Gate 5.5 closure pass, all measured, committed samples): (1) G4 sagitta per arc 17-27 pct of chord, min curvature radius 68-78 px, 2/41 points within 0.5 px of the chord -> arcs, not lines; (2) anticipation_ms measured on the cue clock = 206.1 ms vs drawn hold 206 ms (declared 150-250 confirmed; the strip reading was slow-mo real time + the 140 ms stretch); (3) secondary "ghost impulse" named as follow-through rebound of an underdamped spring (zeta 0.60, half period 292 ms, theory ratio 0.097, observed 0.089, cap 0.15) -> kept, not deleted, as the review asked; (4) G7 edge width in device px: DPR1 rest 3 / flight 2, DPR2 rest 3 / flight 4 (CSS 1.5-3 px), soft-edge and no-temporal-aliasing PASS; (5) perf: p99 + max + env block (2-core Xeon, 1 GB, HeadlessChrome 153); normal CPU 1/3/5 owls median-of-3 jank 0/0/0 pct, p99 16.8 ms; CPU x4 proxy 0.17/1.52/5.25 pct - and the same host at x4 gives v1 4.2 pct and v2-without-Foley 6.46 pct at 5 owls, so the x4 figure is the host ceiling, not an engine regression (disclosed, not hidden); (6) sfx: dropped_by_cue + priority-3 cues (land/chime/aww) never dropped by polyphony (only by the 120 ms same-cue gap, by design); autoplay rerun on a pristine incognito context with userActivation isActive/hasBeenActive logged per step (Chromium automation reports true before any click -> the gate is the AudioContext state, which stayed locked until the click); tVisualEffective measured (performance.now in the first rAF after the write): headed-xvfb median present delay 8-11 ms, effective offset median within the perception window PASS, 75-93 pct of samples within it (disclosed); outputLatency compensation decided NOT applied (would push a 100-200 ms visual lag on Bluetooth onto silent actions) and recorded. Open from Gate 5.5: G8 reference-video split-screen (needs the reference clip from the owner; none in the repo). Next: K9.2 pose sheet gate G11.
- resume-check (reset #31, 2026-09-26): branch @ 3a393ac = K9.1 closed + K9.2a engine/poses.js (PoseLayer additive WAAPI layers, eye-lead stagger, asymmetry, snap/hold/release, validatePoses). Lost: K9.2b (spec "acting" block + six signature poses) - validated locally (validateSpec ok, 0 pose problems) but the commit was interrupted. Redo K9.2b now and commit first; then K9.2c pose sheet tool.
- resume-check (reset #32, 2026-09-26): branch @ 6a530e8 = K9.2 complete on disk (poses.js, spec acting + 6 poses, demo wiring ?acting, pose_sheet.py, g11_pose_sheet.png + 7 stills + json). Lost: only the G11 verdict line (below). Nothing to redo in code.
- K9.2 done (G11 pose sheet gate, stills only, DPR2): sandbox/samples/proofs/g11_pose_sheet.png (+ 7 stills + g11_pose_sheet.json readback of every applied joint transform). Engine: engine/poses.js PoseLayer (additive WAAPI layer per joint, eye-lead stagger 45 ms eyes->head->body, right-side asymmetry 8-45 ms + 18 pct gain jitter, snap/hold/release, freeze for stills) behind ?acting=0; spec "acting" + six "poses"; validatePoses rejects unknown joint / channel / pose (tested: 3 problems on a bad spec, 0 on ours); sw g5-6. HONEST VERDICT per pose (no flattery): alive = 3 launch (weight forward, wings back, legs trailing, smile = intent), 4 puzzled (head tilt 19 deg + one wing raised + half lids + open beak = a question), 6 oops (recoil lean + lids 55 pct + sad beak = "the owl is the surprised one"). Half-alive = 2 charged (crouch + squash read, but half lids look sleepy instead of coiled; needs brows, the art has none), 5 triumph (wings up + open beak read, head tilt too small, eyes cannot widen). Dead = 1 alert (6 deg head tilt + 3 px pupil shift is invisible at this size; indistinguishable from neutral). ROOT CAUSE is the art, not the pose data: eyes.webp is ONE plate for both eyes (pupilR group is empty) so the eyes cannot converge, cross, wink or look apart; lids.webp is ONE plate; there are no brow parts. Half of cartoon expression lives in brows + independent eyes. SINGLE BLOCKER for K9.3+: slice eyes.webp and lids.webp into L/R halves and add two brow parts as raster slices of the existing head art (no redraw, 4 new webp, owl_p2.svg gains 4 groups) - stays inside ADR-002 option A. Without it poses 1/2/5 stay half-alive regardless of physics. Next: owner decision on the slice, then K9.3 beats + intent + eye-lead A/B.
- Blocker feasibility scan (measured, alpha channel): eyes.webp 336x166 and lids.webp 336x166 both have a fully transparent centre gap (columns 159-176, 18 px) -> a left/right slice is lossless, no redraw, no visible seam; the SVG already has empty pupilR / lidR groups waiting for the parts. head.webp is one opaque 340x279 plate with no separable brow region -> brows would need two small NEW raster parts (drawn once, ~40x14 px each) or a vector path, i.e. a real (small) art addition, not a slice. Recommendation to the owner: approve (a) the L/R eye + lid slice now (pure data/asset work, unlocks look-apart, wink, squint, converge) and decide (b) brows separately (needs one drawing pass). K9.3 can proceed on (a) alone; poses 1/2/5 improve fully only with (b).
- STATUS 2026-09-26 (end of session, reset #32): K9.0 ADR-002, K9.1 Gate 5.5 closure (all measured), K9.2 G11 pose sheet DONE and pushed (branch @ 53f77ba). Merge facts (no ownership language): main = 120ce1a (PR #11 merged by the owner account 18:10Z); this branch is 20 commits ahead, NOT merged; app/ diff vs main = 0; Gate 6 closed; flags live (?engine, ?sfx=0, ?acting=0, ?slowmo, ?trace). Preview (temporary): https://8080-ie8q6zyqojhiogqpnkjp7-2e77fc33.sandbox.novita.ai/sandbox/samples/proofs/g11_pose_sheet.png (the six stills) and https://8080-ie8q6zyqojhiogqpnkjp7-2e77fc33.sandbox.novita.ai/sandbox/index.html?engine=v2 (console: __rigs[0].poses.apply('puzzled',{holdMs:600})). WAITING ON OWNER: (a) approve L/R eye+lid slice (lossless, measured 18 px gap), (b) decide brows (new small parts). NEXT (frozen): K9.3 beats + intent + eye-lead A/B on the current art, then K9.4 performances, K9.5 flex, K9.6 G12/G13 + report "Gate 5 cycle 6".
- gist a402871d rev b5d111c1 (2026-09-26T19:40Z, +2 revisions) verified before acting: (1) owner accepts K9.0-K9.2 numbers and corrects his own 475 ms reading - noted. (2) Owner's G11 verdict: alive 2 (puzzled, oops) / half 3 (charged, triumph, alert->fail) / fail launch as "3/4 body turn, no thrust" - ACCEPTED as the governing verdict (owner eye > my eye). (3) New method rule: silhouette first, blur test G11-b, silhouette_delta_pct >= 8 pct (calibrate after first measurement) - accepted. (4) Claim "the beak never moved in the 7 stills, mouth layer has no art" = FALSE on evidence: owl_p2.svg has 5 data-mouth groups over two plates (beak_closed.webp 137x126, beak_open.webp 140x163 with tongue), g11_pose_sheet.json readback shows triumph=open, puzzled=mid, launch=smile, oops=sad, and the triumph still shows the open beak with tongue. The real defect is different and smaller: the beak is 22-24 viewBox units on a 200-unit owl (11 pct of width) so open/closed is unreadable at sheet scale, and smile/sad reuse the closed plate at slightly different offsets (no distinct art). Corrected item (c): not "beak buried in head", but "beak too small + smile/sad have no distinct plate". (5) Decisions received: (a) L/R eye+lid slice APPROVED + extract iris/pupil per eye as its own layer, acceptance pixel_diff = 0 at DPR1/2; (b) brows APPROVED, vector preferred, behind flag; (d) ADR-002 addendum on flex ceiling of raster: mesh-warp renderer vs vector re-author with measured cost; (5) perf table under identical conditions to explain 5.25 vs 6.46 (already flagged by me as host noise; will re-measure interleaved). (6) Order: (a)+(b)+(c)+ADR addendum -> G11 v2 (silhouette first) -> K9.3. Constants unchanged.
- FROZEN PLAN K9.2b (revised after owner decisions; replaces the K9.3-first order):
  K9.2b-1 tools/slice_parts.py: slice eyes.webp/lids.webp at the transparent gap (cols 159-176) into eyesL/eyesR/lidsL/lidsR; extract pupil (iris disc) per eye by colour/alpha mask into pupilL/pupilR plates leaving the white in eyesL/eyesR; write parts.json entries; owl_p2.svg: eyeL/eyeR each get their white + pupil child, lidL/lidR get their plate; prove pixel_diff = 0 by compositing L+R back over the original at DPR1 and DPR2 (recomposite test in the same script) -> samples/proofs/g11b_slice_diff.json.
  K9.2b-2 brows: two vector <path> parts (browL/browR) drawn from the head art's brow ridge colour, data-joint with pivot at the inner end; spec joints list + poses gain brow channels; flag ?brows=0 (and spec.acting.brows) -> zero regression when off.
  K9.2b-3 beak readability: (i) measure beak share of head width vs cartoon references (Duo ~28-32 pct of head width, ours 11 pct) and propose a scale in the spec (mouth.scale) applied to the mouth joint - data only; (ii) mark smile/sad as "no distinct art" in the layer inventory; propose a beak_smile plate later (art). Layer inventory table -> docs/ART_INVENTORY.md.
  K9.2b-4 ADR-002 addendum: flex ceiling of raster parts; options mesh-warp Canvas2D (measured: prototype 1 wing quad warp, cost in ms per frame at 5 owls) vs vector re-author (time estimate, who) vs stay raster+joints; recommendation; owner decides.
  K9.2b-5 G11 v2: redo the six poses silhouette-first (body lean, wing asymmetry, leg spread, head thrust) using the new eye/brow parts; pose_sheet.py adds silhouette_delta_pct (alpha mask XOR vs neutral / neutral area) and a blur-test strip (gaussian 6 px); gate: every pose >= 8 pct delta (calibrate) and readable blurred.
  K9.2b-6 perf table identical conditions: 5 owls, same scene, 10 s, 3 interleaved runs each of v2+Foley / v2 no-Foley / v1 at CPU x1 and x4 -> samples/measure_matrix.json.
  Then K9.3 beats + intent + eye-lead A/B (unchanged).
- 2026-09-26 resume-check #33 (after sandbox reset): branch sandbox/a1-gate5-art-sw at 50d3783 == origin, clean tree; origin/main 120ce1a; app/ diff vs main = 0; protected files untouched. Last recorded point = FROZEN PLAN K9.2b-1..6. Reality check: no sandbox/tools/slice_parts.py, no samples/proofs/g11b_slice_diff.json, parts dir still 9 webp + parts.json -> K9.2b-1 not started (record and reality agree, nothing to redo). Executing K9.2b-1 now (iris circle-fit extraction, L/R slices, parts.json + owl_p2.svg update, recomposite pixel_diff proof at DPR1/DPR2).
- 2026-09-26 K9.2b-1 DONE (commits 85daafc tool, 02df388 slices, db81180 + 96c2267 proofs): sandbox/tools/slice_parts.py (deterministic, idempotent; reruns produce byte-identical outputs). Facts: transparent gap cols 161-174 (eyes) / cut col 168; iris circle fit (Kasa LSQ + residual outlier rejection, tol 2 px): L centre (87.11, 82.96) r 50.29 rms 0.36 px, R centre (246.98, 82.94) r 50.39 rms 0.38 px; disc cut at r+2.5 px (measured anti-aliased rim: sat 0.19 at r+1, 0.12 at r+2, sclera 0.09); white plates get a smooth sclera inpaint (band mean r+3..r+7 at 720 angles, circular gaussian 12 deg, quadratic radial blend). New parts (lossless webp): eyesL 21006 B, eyesR 20626, pupilL 10548, pupilR 10542, lidsL 21558, lidsR 21564 (active total 190046 B vs 113220 before; lossless is the price of pixel_diff = 0, disclosed in the json). owl_p2.svg: eyeL/eyeR = white + pupil child, lidL/lidR = own plate, pivots at each iris centre (81.29, 85.19) / (118.27, 85.19), image rects reproduce the original xMidYMid-meet rect exactly, preserveAspectRatio none. Proof samples/proofs/g11b_slice_diff.json: plate recomposite eyes+lids max_abs_diff 0 / 0 px; rendered orig-vs-new (Playwright, lids forced visible, 3 renders per doc, per-pixel median) DPR1 0/192000 px, DPR2 0/768000 px; raw pairs and same-doc controls listed unfiltered (DPR1 shows an intermittent 46-px one-column flicker at css x=97 = wingL, also orig-vs-orig -> renderer noise, never inside the eye region). Live check: 0 console errors, __specCheck ok, pupils translate over intact white (g11b_live_neutral_look_puzzled.png), lids scaleY per eye. Regression: proofs.py exit 0 (G3/G4/G5/G7 pass, 3 runs), sw g5-7 precaches new parts. NEXT: K9.2b-2 vector brows behind flag (?brows=0 / spec.acting.brows).
- 2026-09-26 resume-check #34 (after sandbox reset): branch sandbox/a1-gate5-art-sw at ec91bd4 == origin, clean; origin/main 120ce1a; app/ diff 0. Record says K9.2b-1 DONE -> reality: slice_parts.py, g11b_slice_diff.json, 6 sliced webp present; no brow joint/flag anywhere -> K9.2b-2 not started, agrees. Gist a402871d: latest revision still b5d111c1 (19:40Z), no new directive. Executing K9.2b-2 (vector brows behind flag) - measure head plate first, no guessing.
- 2026-09-26 K9.2b-2 DONE (dcbc6ec feature, 103071d proof): vector brows. Inputs measured, not guessed: eye centres from K9.2b-1 circle fit, eye disc top 65.8 u, head.webp brow band colours (p10 lum rgb 89,44,23 body; p40-60 rgb 186,132,84 highlight at 0.55), mirror axis 99.78. SVG: <g data-feature="brows" style="display:none"> with data-joint browL/browR (2 <path> each, pivots (81.29,64.2)/(118.27,64.2)). Flag = URL ?brows=0 AND spec.acting.brows === true (both required to show); poses.js eyes lead-group includes brows; validatePoses knows browL/browR; every pose got rot/y brow channels (asymmetric for puzzled/oops). Proof samples/proofs/g11b_brows_flag.json (brows_proof.py): A flag off vs pre-brow SVG 55934bc -> DPR1 0 px, DPR2 0 px (3 renders/doc median, raw pairs 0, computed display none); B flag on -> 1216 px differ, 100 pct inside brow band (55-67 u), 0 console errors, specCheck ok, browL/browR transforms change on apply('puzzled'); C spec gate false -> hidden. Stars removed inside the proof because they are Math.random positioned. Regression: pose_sheet.py ok (brows visible in all 6 poses), sw g5-8. NEXT: K9.2b-3 beak readability (measure beak share vs cartoon refs, propose mouth.scale) + docs/ART_INVENTORY.md.
