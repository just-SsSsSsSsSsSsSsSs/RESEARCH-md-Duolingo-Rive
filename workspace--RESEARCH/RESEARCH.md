...
## 🌟 إضافات وتطويرات الجيل الجديد المقترحة لبوابة أبطال البيت (2026 Edition)

### 1. 🫧 نظام الفقاعات التفاعلية الحية (Dynamic Floating & Interactive Bubbles)
- **المفهوم:** خلفية حية تفاعلية بفقاعات نيون ملونة تطفو بنعومة (Floating Particles) وتتفاعل مع حركة الماوس أو لمس شاشة الموبايل (Touch Interactive).
- **التفاعل الصوتي:** عند لمس أو فرقعة أي فقاعة، تصدر صوت "Pop" مبهج ومحبب للأطفال باستخدام `Web Audio API` (توليد برمجي نقي خفيف وفوري بدون ملفات صوتية خارجية).
- **تأثيرات الجاذبية والحركة:** حركة فيزيائية سلسة (Canvas 2D / CSS Canvas) بمعدل 60 إطار في الثانية دون استهلاك المعالج أو البطارية.

### 2. 🎨 مكتبة أيقونات عصرية فائقة الحداثة (Modern Iconography & SVG Micro-Animations)
- **المكتبات الموصى بها:** `Remix Icon v4.x` مع `Lucide Icons` و `SVG Morphing`.
- **التأثيرات الحركية:** أيقونات تنبض وتتفاعل عند الـ Hover والـ Tap، مع تأثيرات إضاءة نيون (Glow Effects) ودرجات تدرج زجاجية (Glassmorphism & Cyber Neon).

### 3. ⚡ تقنيات الويب المتقدمة (Full-Stack Modern Capabilities)
- **Vanilla JS + Web Components:** بناء مكونات UI مرنة وسريعة بدون أطر عمل ثقيلة، قابلة للعمل المباشر على GitHub Pages وأي سيرفر محلي.
- **Server-Sent Events (SSE) & Local Storage Synchronization:** لمزامنة إنجازات الطفل ولوحة الشرف والشهادات محلياً وسحابياً.
- **Smooth Inertia Scroll & Micro-Interactions:** تجربة تمرير سلسة وقوائم تبويب تتنقل بنعومة فائقة بين أقسام المنصة (القرآن، الرياضيات، القصص، البودكاست).

### 4. 🔗 رادار المنصات المفتوحة والمشاريع الإلهامية المستمرة
- **منصات تعليمية ملهمة:**
  - [OpenEduCat](https://github.com/openeducat/openeducat_erp) — إدارة تعلم متكاملة
...
# Phase 18 - بحث «الحركة الحية الانسيابية» للرفقاء (2026-09-25)

توجيه المالك (رابط العيلة): «الانتقال من تبديل 4 لقطات ثابتة إلى حركة انسيابية مرنة مستمرة (تنفس، رمش، قفز، تفاعل مع اللمس) على غرار Duolingo وRive» + «مش عايزين حاجة ثابتة» + استكمال النحلة/السلحفاة/الروبوت + النظام مفتوح لأبطال جدد.

## 1. ما الذي يحدث فعلاً اليوم (Phase 17)
- الرفيق = `<img>` واحدة لكل وضعية، مكدّسة وتُبدَّل بـopacity (0.18s). كل الحركة transform على `.cp-stage` **للجسم كله** ككتلة واحدة.
- الهدوء = «كيس» أفعال منفصلة (breath/blink/glance/hop/lean/wiggle) بينها **فجوات سكون 0.5-2.3 ث** -> الشخصية تبدو ثابتة بين الأفعال.
- الانتقال بين الوضعيات = قطع (hard swap) بلا squash/stretch ولا overshoot؛ easing واحد `cubic-bezier(.34,1.56,.64,1)`.
- لا تفاعل مع اللمس (pointer-events: none)، ولا تفاعل مع ما يفعله الطفل قبل الإجابة.

## 2. ما تفعله Duolingo/Rive فعلاً (المبدأ لا الأداة)
- **طبقات (rig):** الجسم والرأس والعينان طبقات منفصلة تتحرك بتأخير طفيف (secondary motion) -> الإحساس بالحياة يأتي من عدم تزامن الطبقات، لا من عدد اللقطات.
- **حلقة هدوء مستمرة** (breathing loop) لا تتوقف أبداً، تُركَّب عليها أفعال عشوائية.
- **فيزياء زنبرك** للانتقالات: overshoot ثم استقرار.
- **تفاعل بالمدخلات:** الشخصية تنظر لما يلمسه المستخدم وتتفاعل عند لمسها.

## 3. الخيارات المفحوصة
| الخيار | الدليل | الحكم |
|---|---|---|
| Rive/Lottie | كما في Phase 17: تبعية 40-60KB + أدوات تأليف غير متاحة | مرفوض (ثابت) |
| إعادة رندر الرفقاء كطبقات منفصلة | 3-4 رندرات x 5 وضعيات x 4-7 رفقاء = 60-140 صورة؛ الرصيد نفد مرتين | مرفوض الآن |
| **تقسيم الصورة الحالية لطبقتين بـCSS mask** | نفس `<img>` تُرسم مرتين بـ`mask-image: linear-gradient()` ناعم (رأس = أعلى ~45%، جسم = أسفل ~62%، تداخل ~8%)؛ لكل طبقة animation مستقلة. لا أصول جديدة؛ compositor-only؛ التداخل بنفس البكسلات فلا خط قطع عند إزاحات ≤ 3px. دعم mask-image: كل المتصفحات الحديثة (Safari بـ-webkit-). رجوع تلقائي لطبقة واحدة عند عدم الدعم. | **مقبول - القرار** |
| **زنبرك عبر `linear()` easing** | WAAPI يدعم `easing: linear(0, .3, 1.2, .9, 1.02, 1)`؛ زنبرك مخمَّد يُعيَّن في JS إلى 40 نقطة. دعم: Chrome 113+، Firefox 112+، Safari 17.2+. رجوع: `cubic-bezier` الحالي عند `!CSS.supports('animation-timing-function','linear(0,1)')`. | مقبول |
| **حلقة تنفس لا نهائية + رأس متأخر** | `iterations: Infinity, direction: alternate` على الجسم (scaleY 1->1.028، 3.2s) وعلى الرأس بتأخير 120ms؛ الأفعال العشوائية تُركَّب فوقها بـ`composite: 'add'` -> لا لحظة سكون. | مقبول |
| **squash & stretch عند تبديل الوضعية** | عند show(): scale(1.06,.94) -> (.97,1.03) -> (1,1) خلال 260ms بزنبرك، composite add. | مقبول |
| **تفاعل اللمس (tickle)** | pointerdown على الرفيق -> react('tickle') + انفجار قلوب صغير من fx؛ `stopPropagation + preventDefault` فلا يصل لأي زر؛ يبقى aria-hidden وغير قابل للتركيز. تعديل موثّق لقاعدة «pointer-events none» -> «اللمس لا يجيب ولا يتنقل ولا يغطي زراً». | مقبول (قرار في AGENTS + CHANGELOG) |
| **النظر لما يلمسه الطفل (lookAt)** | pointerdown على `.choice/.numpad .btn` -> الرأس يدور حتى 9 درجات نحو نقطة اللمس 900ms ثم يعود. | مقبول |
| **الترقّب (anticipation)** | أول رقم على numpad -> ميل للأمام 3 درجات يثبت حتى الإجابة. | مقبول |
| نحلة/سلحفاة/روبوت | ورقة واحدة لكل شخصية ثم `tools/cut_sprite_sheet.py`؛ محاولة واحدة لكل جلسة وإلا توثيق (قاعدة الرصيد). | مقبول |
