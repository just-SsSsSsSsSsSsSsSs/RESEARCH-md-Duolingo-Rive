...
export async function render(root) {
  const h = hud({ back: true, title: '🏆 الشارات' });
  root.appendChild(h);
  ...
  root.appendChild(el(`<div class="card center glow-gold">
    <div style="font-size:56px" class="float">🏆</div>
  ...
  for (const t of [1, 2, 3]) {
    ...
    root.appendChild(el(`<div class="section"><h2>${t === 3 ? '👑' : t === 2 ? '🥈' : '🥉'} شارات ${TIER[t][0]}</h2><span class="tag ${TIER[t][1]}">${fmt(group.filter((b) => b.earned).length)}/${fmt(group.length)}</span></div>`));
