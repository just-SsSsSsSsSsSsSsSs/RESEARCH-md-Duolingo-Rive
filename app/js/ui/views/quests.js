  ...
  const draw = () => {
    ...
    list.forEach((q, i) => {
      ...
      const card = el(`<div class="card ${q.done ? 'glow-green' : ''}">
        <div class="row">
          <div class="icon-box" style="width:52px;height:52px;font-size:26px;border-radius:14px;background:rgba(255,255,255,.06);display:grid;place-items:center">${ico3d(q.icon, 32)}</div>
          <div class="grow">
      ...
      card.querySelector('button')?.addEventListener('click', (e) => { if (quests.claim(i)) { confetti({ count: 70, x: e.clientX, y: e.clientY }); sound.play('coin'); toast(`${ico3d('star')} +${fmt(q.reward)} XP و 3 ${ico3d('gem')}`, { type: 'gold' }); draw(); } });
      wrap.appendChild(card);
    });
    if (list.every((q) => q.claimed)) wrap.appendChild(el('<div class="card center"><div style="font-size:40px">' + ico3d('party') + '</div><b>أنهيت كل مهام اليوم!</b><p class="muted small">ارجع غداً لمهام جديدة</p></div>'));
  };
  draw();
