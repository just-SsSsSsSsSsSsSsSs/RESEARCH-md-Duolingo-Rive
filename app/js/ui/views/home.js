...
export async function render(root) {
  ...
  root.appendChild(el(`
    <div class="card" style="border-color:${hero.hex}55;background:linear-gradient(140deg, ${hero.hex}22, var(--surface))">
      <div class="row">
        <div style="font-size:56px" class="float">${p.emoji}</div>
        <div class="grow">
          <h1 style="font-size:24px">${greet} يا ${esc(p.name)}! 👋</h1>
          <p class="muted">${p.streak.count ? `شعلتك ${fmt(p.streak.count)} يوم 🔥 — كمّل النهاردة!` : 'ابدأ نشاطاً واحداً لتشعل شعلتك 🔥'}</p>
        </div>
      </div>
    </div>`));

  if (rec) {
    root.appendChild(el(`<div class="section"><h2>✨ مقترح لك الآن</h2><span class="tag tag-purple">ذكي</span></div>`));
    root.appendChild(itemCard(rec, { big: true }));
  }

  root.appendChild(el(`<div class="section"><h2>🎯 مهام اليوم</h2><span class="tag ${done === q.length ? 'tag-green' : 'tag-cyan'}">${fmt(done)}/${fmt(q.length)}</span></div>`));
  root.appendChild(el(`<a href="#/quests" class="card clickable stack" style="gap:8px">
    ${q.map((x) => `<div class="row"><span style="font-size:22px">${x.icon}</span><div class="grow"><div class="small" style="font-weight:800">${esc(x.name)}</div><div class="level-bar green" style="height:6px;margin-top:4px"><span style="width:${Math.round((x.prog / x.target) * 100)}%"></span></div></div><span class="tag ${x.done ? 'tag-green' : 'tag-gold'}">${x.done ? '✓' : `+${fmt(x.reward)}`}</span></div>`).join('')}
  </a>`));

  root.appendChild(el(`<div class="section"><h2>📚 المواد</h2></div>`));
  ...
  for (const s of registry.subjects()) {
    ...
    const c = el(`<a href="#/subject/${s.id}" class="card clickable center" style="border-color:${s.color}55;padding:18px 10px">
      <div style="font-size:40px;line-height:1.2" class="float">${s.icon}</div>
  ...
  if (recent.length) {
    root.appendChild(el(`<div class="section"><h2>🕘 آخر ما لعبت</h2></div>`));
    const g = el('<div class="grid-2"></div>');
    recent.forEach((it) => g.appendChild(itemCard(it)));
    root.appendChild(g);
  }

  root.appendChild(el(`<p class="center small muted mt-6">❤️ منصة العائلة لأبطال البيت • تعمل بدون إنترنت</p>`));
